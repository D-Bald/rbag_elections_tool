defmodule RbagElectionsWeb.AdminLoginLiveTest do
  ...
  describe "Log in page" do
    test "renders log in page", %{conn: conn} do
      {:ok, _lv, html} = live(conn, ~p"/admins/login")
  ...
  describe "admin login" do
    test "redirects if admin login with valid credentials", %{conn: conn} do
      ...
      {:ok, lv, _html} = live(conn, ~p"/admins/login")
    ...
    test "redirects to login page with a flash error if there are no valid credentials", %{
      ...
      assert redirected_to(conn) == "/admins/login"
    end
  end
