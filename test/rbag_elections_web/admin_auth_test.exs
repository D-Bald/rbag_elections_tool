defmodule RbagElectionsWeb.AdminAuthTest do
  use RbagElectionsWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias RbagElections.Management
  alias RbagElectionsWeb.AdminAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RbagElectionsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_admin/3" do
    test "stores the admin token in the session", %{conn: conn, admin: admin} do
      ...
      assert Management.get_admin_by_session_token(token)
    end
  ...
  describe "logout_admin/1" do
    test "erases session and cookies", %{conn: conn, admin: admin} do
      admin_token = Management.generate_admin_session_token(admin)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Management.get_admin_by_session_token(admin_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "admins_sessions:abcdef-token"
      RbagElectionsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :ensure_authenticated" do
    ...
    test "redirects to login page if there isn't a valid admin_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: RbagElectionsWeb.Endpoint,
