defmodule RbagElections.ManagementTest do
  ...
  describe "get_admin_by_email_and_password/2" do
    ...
    test "returns the admin if the email and password are valid" do
      ...
      assert %Admin{id: ^id} =
               Management.get_admin_by_email_and_password(admin.email, valid_admin_password())
    end
  end
  ...
  describe "get_admin!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Management.get_admin!(-1)
      end
    end

    test "returns the admin with the given id" do
      %{id: id} = admin = admin_fixture()
      assert %Admin{id: ^id} = Management.get_admin!(admin.id)
    end
  end

  describe "register_admin/1" do
    test "requires email and password to be set" do
      {:error, changeset} = Management.register_admin(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = Management.register_admin(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = Management.register_admin(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Management.register_admin(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = Management.register_admin(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers admins with a hashed password" do
      email = unique_admin_email()
      {:ok, admin} = Management.register_admin(valid_admin_attributes(email: email))
  ...
  describe "change_admin_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = Management.change_admin_registration(%Admin{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Management.change_admin_registration(
          %Admin{},
  ...
  describe "change_admin_email/2" do
    test "returns a admin changeset" do
      assert %Ecto.Changeset{} = changeset = Management.change_admin_email(%Admin{})
  ...
  describe "apply_admin_email/3" do
    ...
    test "requires email to change", %{admin: admin} do
      {:error, changeset} = Management.apply_admin_email(admin, valid_admin_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{admin: admin} do
      {:error, changeset} =
        Management.apply_admin_email(admin, valid_admin_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{admin: admin} do
      ...
      {:error, changeset} =
        Management.apply_admin_email(admin, valid_admin_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{admin: admin} do
      ...
      {:error, changeset} = Management.apply_admin_email(admin, password, %{email: email})
    ...
    test "validates current password", %{admin: admin} do
      {:error, changeset} =
        Management.apply_admin_email(admin, "invalid", %{email: unique_admin_email()})
    ...
    test "applies the email without persisting it", %{admin: admin} do
      email = unique_admin_email()
      {:ok, admin} = Management.apply_admin_email(admin, valid_admin_password(), %{email: email})
      assert admin.email == email
      assert Management.get_admin!(admin.id).email != email
    end
  end
    ...
    test "sends token through notification", %{admin: admin} do
      token =
        extract_admin_token(fn url ->
          Management.deliver_admin_update_email_instructions(admin, "current@example.com", url)
        end)
  ...
  describe "update_admin_email/2" do
    ...
    test "updates the email with a valid token", %{admin: admin, token: token, email: email} do
      assert Management.update_admin_email(admin, token) == :ok
    ...
    test "does not update email with invalid token", %{admin: admin} do
      assert Management.update_admin_email(admin, "oops") == :error
    ...
    test "does not update email if admin email changed", %{admin: admin, token: token} do
      assert Management.update_admin_email(%{admin | email: "current@example.com"}, token) == :error
      assert Repo.get!(Admin, admin.id).email == admin.email
      assert Repo.get_by(AdminToken, admin_id: admin.id)
    end
    ...
    test "does not update email if token expired", %{admin: admin, token: token} do
      ...
      assert Management.update_admin_email(admin, token) == :error
      assert Repo.get!(Admin, admin.id).email == admin.email
      assert Repo.get_by(AdminToken, admin_id: admin.id)
    end
  end
  ...
  describe "change_admin_password/2" do
    test "returns a admin changeset" do
      assert %Ecto.Changeset{} = changeset = Management.change_admin_password(%Admin{})
      assert changeset.required == [:password]
    end

    test "allows fields to be set" do
      changeset =
        Management.change_admin_password(%Admin{}, %{
  ...
  describe "update_admin_password/3" do
    ...
    test "validates password", %{admin: admin} do
      {:error, changeset} =
        Management.update_admin_password(admin, valid_admin_password(), %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{admin: admin} do
      ...
      {:error, changeset} =
        Management.update_admin_password(admin, valid_admin_password(), %{password: too_long})
    ...
    test "validates current password", %{admin: admin} do
      {:error, changeset} =
        Management.update_admin_password(admin, "invalid", %{password: valid_admin_password()})
    ...
    test "updates the password", %{admin: admin} do
      ...
      assert is_nil(admin.password)
      assert Management.get_admin_by_email_and_password(admin.email, "new valid password")
    end

    test "deletes all tokens for the given admin", %{admin: admin} do
      _ = Management.generate_admin_session_token(admin)
  ...
  describe "generate_admin_session_token/1" do
    ...
    test "generates a token", %{admin: admin} do
      token = Management.generate_admin_session_token(admin)
  ...
  describe "get_admin_by_session_token/1" do
    ...
    test "returns admin by token", %{admin: admin, token: token} do
      assert session_admin = Management.get_admin_by_session_token(token)
      assert session_admin.id == admin.id
    end

    test "does not return admin for invalid token" do
      refute Management.get_admin_by_session_token("oops")
    end

    test "does not return admin for expired token", %{token: token} do
      {1, nil} = Repo.update_all(AdminToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Management.get_admin_by_session_token(token)
    end
  end
  ...
  describe "delete_admin_session_token/1" do
    test "deletes the token" do
      admin = admin_fixture()
      token = Management.generate_admin_session_token(admin)
      assert Management.delete_admin_session_token(token) == :ok
      refute Management.get_admin_by_session_token(token)
    end
  end
  ...
  describe "deliver_admin_confirmation_instructions/2" do
    ...
    test "sends token through notification", %{admin: admin} do
      token =
        extract_admin_token(fn url ->
          Management.deliver_admin_confirmation_instructions(admin, url)
        end)
  ...
  describe "confirm_admin/1" do
    ...
    test "confirms the email with a valid token", %{admin: admin, token: token} do
      assert {:ok, confirmed_admin} = Management.confirm_admin(token)
    ...
    test "does not confirm with invalid token", %{admin: admin} do
      assert Management.confirm_admin("oops") == :error
    ...
    test "does not confirm email if token expired", %{admin: admin, token: token} do
      {1, nil} = Repo.update_all(AdminToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert Management.confirm_admin(token) == :error
      refute Repo.get!(Admin, admin.id).confirmed_at
      assert Repo.get_by(AdminToken, admin_id: admin.id)
    end
  end
  ...
  describe "deliver_admin_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{admin: admin} do
      token =
        extract_admin_token(fn url ->
          Management.deliver_admin_reset_password_instructions(admin, url)
        end)
  ...
  describe "get_admin_by_reset_password_token/1" do
    ...
    test "returns the admin with valid token", %{admin: %{id: id}, token: token} do
      assert %Admin{id: ^id} = Management.get_admin_by_reset_password_token(token)
      assert Repo.get_by(AdminToken, admin_id: id)
    end

    test "does not return the admin with invalid token", %{admin: admin} do
      refute Management.get_admin_by_reset_password_token("oops")
      assert Repo.get_by(AdminToken, admin_id: admin.id)
    end

    test "does not return the admin if token expired", %{admin: admin, token: token} do
      {1, nil} = Repo.update_all(AdminToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Management.get_admin_by_reset_password_token(token)
      assert Repo.get_by(AdminToken, admin_id: admin.id)
    end
  end
  ...
  describe "reset_admin_password/2" do
    ...
    test "validates password", %{admin: admin} do
      {:error, changeset} =
        Management.reset_admin_password(admin, %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{admin: admin} do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = Management.reset_admin_password(admin, %{password: too_long})
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end

    test "updates the password", %{admin: admin} do
      {:ok, updated_admin} = Management.reset_admin_password(admin, %{password: "new valid password"})
    ...
    test "deletes all tokens for the given admin", %{admin: admin} do
      _ = Management.generate_admin_session_token(admin)
      {:ok, _} = Management.reset_admin_password(admin, %{password: "new valid password"})
      refute Repo.get_by(AdminToken, admin_id: admin.id)
    end
  end
