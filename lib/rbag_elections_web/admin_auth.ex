defmodule RbagElectionsWeb.AdminAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_rbag_elections_web_admin_remember_me"
  ...
  def log_out_admin(conn) do
    ...
    admin_token && Management.delete_admin_session_token(admin_token)
  ...
  def fetch_current_admin(conn, _opts) do
    ...
    admin = admin_token && Management.get_admin_by_session_token(admin_token)
  ...
  the current_admin:
      ...
      defmodule RbagElectionsWeb.PageLive do
        ...
        on_mount {RbagElectionsWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{RbagElectionsWeb.AdminAuth, :ensure_authenticated}] do
  ...
  def on_mount(:ensure_authenticated, _params, session, socket) do
    ...
    if socket.assigns.current_admin do
      ...
      socket =
        ...
        |> Phoenix.LiveView.redirect(to: ~p"/admins/login")
  ...
  defp mount_current_admin(socket, session) do
    Phoenix.Component.assign_new(socket, :current_admin, fn ->
      if admin_token = session["admin_token"] do
        Management.get_admin_by_session_token(admin_token)
      end
    end)
  end
