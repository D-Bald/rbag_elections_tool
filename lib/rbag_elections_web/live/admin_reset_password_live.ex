defmodule RbagElectionsWeb.AdminResetPasswordLive do
  ...
  def handle_event("reset_password", %{"admin" => admin_params}, socket) do
    case Management.reset_admin_password(socket.assigns.admin, admin_params) do
      {:ok, _} ->
        {:noreply,
         socket
  ...
  def handle_event("validate", %{"admin" => admin_params}, socket) do
    changeset = Management.change_admin_password(socket.assigns.admin, admin_params)
    {:noreply, assign_form(socket, Map.put(changeset, :action, :validate))}
  end

  defp assign_admin_and_token(socket, %{"token" => token}) do
    if admin = Management.get_admin_by_reset_password_token(token) do
